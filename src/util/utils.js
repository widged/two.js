...
var sin = Math.sin,
    ...
    min = Math.min,
    max = Math.max;
var Utils = _underscore2['default'].extend(_underscore2['default'], {
  ...
  release: function release(obj) {
    ...
    if (_underscore2['default'].isFunction(obj.unbind)) {
      obj.unbind();
    }

    if (obj.vertices) {
      if (_underscore2['default'].isFunction(obj.vertices.unbind)) {
        obj.vertices.unbind();
      }
      _underscore2['default'].each(obj.vertices, function (v) {
        if (_underscore2['default'].isFunction(v.unbind)) {
          v.unbind();
        }
      });
    }
    ...
    if (obj.children) {
      _underscore2['default'].each(obj.children, function (obj) {
        Utils.release(obj);
      });
    }
  },
  ...
  getComputedMatrix: function getComputedMatrix(object, matrix) {
    ...
    _underscore2['default'].each(matrices, function (m) {
  ...
  deltaTransformPoint: function deltaTransformPoint(matrix, x, y) {
   ...
   * of any given path.
   */
  applySvgAttributes: function applySvgAttributes(node, elem) {
    ...
    if (!_underscore2['default'].isUndefined(styles.opacity)) {
      styles['stroke-opacity'] = styles.opacity;
      styles['fill-opacity'] = styles.opacity;
    }

    // Merge attributes and applied styles (attributes take precedence)
    _underscore2['default'].extend(styles, attributes);

    // Similarly visibility is influenced by the value of both display and visibility.
    // Calculate a unified value here which defaults to `true`.
    styles.visible = !(_underscore2['default'].isUndefined(styles.display) && styles.display === 'none') || _underscore2['default'].isUndefined(styles.visibility) && styles.visibility === 'hidden';
  ...
  read: {
    ...
    polygon: function polygon(node, open) {
    ...
    path: function path(node) {
      ...
      _underscore2['default'].each(commands.slice(0), function (command, i) {
      ...
      var points = [];
      _underscore2['default'].each(commands, function (command, i) {
        ...
        switch (lower) {
          ...
          case 'z':
            if (i >= last) {
              ...
              x = coord.x;
              y = coord.y;
              result = new Anchor(x, y, undefined, undefined, undefined, undefined, _constantCommandTypes2['default'].close);
            }
            break;
          ...
          case 'l':
            ...
            result = new Anchor(x, y, undefined, undefined, undefined, undefined, lower === 'm' ? _constantCommandTypes2['default'].move : _constantCommandTypes2['default'].line);
          ...
          case 'v':
            ...
            result = new Anchor(undefined, undefined, undefined, undefined, undefined, undefined, _constantCommandTypes2['default'].line);
          ...
          case 's':
            ...
            if (!_underscore2['default'].isObject(coord.controls)) {
              Anchor.AppendCurveProperties(coord);
            }

            coord.controls.right.set(x2 - coord.x, y2 - coord.y);
            result = new Anchor(x4, y4, x3 - x4, y3 - y4, undefined, undefined, _constantCommandTypes2['default'].curve);
          ...
          case 'a':
            ...
            if (_underscore2['default'].isNaN(amp)) {
              amp = 0;
            ...
            var projection = new Matrix().translate(cx, cy).rotate(xAxisRotation);

            // Create a resulting array of Anchor's to export to the
            // the path.
            result = _underscore2['default'].map(_underscore2['default'].range(length), function (i) {
              var pct = 1 - i / (length - 1);
              var theta = pct * dt + t1;
              var x = rx * Math.cos(theta);
              var y = ry * Math.sin(theta);
              var projected = projection.multiply(x, y, 1);
              return new Anchor(projected.x, projected.y, false, false, false, false, _constantCommandTypes2['default'].line);;
            });

            result.push(new Anchor(x4, y4, false, false, false, false, _constantCommandTypes2['default'].line));
    ...
    circle: function circle(node) {
      ...
      var amount = _constantResolution2['default'];
      var points = _underscore2['default'].map(_underscore2['default'].range(amount), function (i) {
        var pct = i / amount;
    ...
    rect: function rect(node) {
      ...
      var points = [new Anchor(w2, h2), new Anchor(-w2, h2), new Anchor(-w2, -h2), new Anchor(w2, -h2)];
    ...
    line: function line(node) {
      ...
      var points = [new Anchor(-w2, -h2), new Anchor(w2, h2)];
    ...
    lineargradient: function lineargradient(node) {
      ...
      for (var i = 0; i < node.children.length; i++) {
        ...
        if (_underscore2['default'].isNull(color)) {
          var matches = style.match(/stop\-color\:\s?([\#a-fA-F0-9]*)/);
          color = matches && matches.length > 1 ? matches[1] : undefined;
        }

        if (_underscore2['default'].isNull(opacity)) {
    ...
    radialgradient: function radialgradient(node) {
      ...
      if (_underscore2['default'].isNaN(fx)) {
        fx = cx;
      }

      if (_underscore2['default'].isNaN(fy)) {
        fy = cy;
      }
  ...
  subdivide: function subdivide(x1, y1, x2, y2, x3, y3, x4, y4, limit) {
    ...
    return _underscore2['default'].map(_underscore2['default'].range(0, amount), function (i) {
  ...
  getPointOnCubicBezier: function getPointOnCubicBezier(t, a, b, c, d) {
    var k = 1 - t;
  ...
  getCurveLength: function getCurveLength(x1, y1, x2, y2, x3, y3, x4, y4, limit) {
    ...
    return integrate(integrand, 0, 1, limit || Utils.Curve.RecursionLimit);
  },
  ...
  getCurveFromPoints: function getCurveFromPoints(points, closed) {
    ...
    for (var i = 0; i < l; i++) {
      ...
      if (!_underscore2['default'].isObject(point.controls)) {
      ...
      b._command = i === 0 ? _constantCommandTypes2['default'].move : _constantCommandTypes2['default'].curve;

      b.controls.left.x = _underscore2['default'].isNumber(b.controls.left.x) ? b.controls.left.x : b.x;
      b.controls.left.y = _underscore2['default'].isNumber(b.controls.left.y) ? b.controls.left.y : b.y;

      b.controls.right.x = _underscore2['default'].isNumber(b.controls.right.x) ? b.controls.right.x : b.x;
      b.controls.right.y = _underscore2['default'].isNumber(b.controls.right.y) ? b.controls.right.y : b.y;
    }
  },
  ...
  getControlPoints: function getControlPoints(a, b, c) {
    ...
    b.u = _underscore2['default'].isObject(b.controls.left) ? b.controls.left : new Vector(0, 0);
    b.v = _underscore2['default'].isObject(b.controls.right) ? b.controls.right : new Vector(0, 0);
  ...
  getAnchorsFromArcData: function getAnchorsFromArcData(center, xAxisRotation, rx, ry, ts, td, ccw) {
    ...
    var matrix = new Matrix().translate(center.x, center.y).rotate(xAxisRotation);

    var l = _constantResolution2['default'];

    // console.log(arguments);

    return _underscore2['default'].map(_underscore2['default'].range(l), function (i) {
      ...
      anchor.command = _constantCommandTypes2['default'].line;
  ...
  ratioBetween: function ratioBetween(A, B) {
  ...
  distanceBetweenSquared: function distanceBetweenSquared(p1, p2) {
