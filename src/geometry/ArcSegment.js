...
function arcSegment(ox, oy, ir, or, sa, ea, res) {
  ...
  var command = _constantCommandTypes2['default'].move;
  var points = [];

  points.push([Math.sin(0) * or, Math.cos(0) * or, 0, 0, 0, 0, command]);

  var theta, x, y, lx, ly, rx, ry;
  command = _constantCommandTypes2['default'].curve;
  ...
  for (var i = 0; i < res + 1; i++) {
    ...
    points.push([x, y, lx, ly, rx, ry, command]);
  }
  ...
  return { points: points, rotation: sa, translation: [ox, oy] };
}

exports['default'] = arcSegment;
